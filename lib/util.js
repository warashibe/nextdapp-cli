...
var updateProps = function updateProps(_ref2) {
  ...
  for (var pre in plugins) {
    ...
    var json = plugins[pre];
    var src = json.noinstall === true ? "../" + json.path : json.name;
    props.push("import { default as ".concat(pre, " } from \"").concat(src, "/lib/init\""));
    props.push("mergeProps(\"".concat(pre, "\", ").concat(pre, ")"));
  }
...
var getJSON = function getJSON(_ref3) {
  ...
  var json_path = xNil(tar_path) ? resolve("".concat(tar_path, "/nextdapp.json")) : resolve("node_modules/".concat(name, "/nextdapp.json"));
  var json = null;
