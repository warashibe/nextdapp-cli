...
var updateFuncs = function updateFuncs(_ref) {
  ...
  for (var _pre in plugins) {
    var exp = [];
    console.log("checking plugin funcs...");
    console.log();
    var json = plugins[_pre];
    var src = "nd/".concat(_pre);
    ...
    try {
      for (_iterator.s(); !(_step = _iterator.n()).done;) {
        var v = _step.value;
        var ns = json.namespace !== null ? "$".concat(json.namespace) : json.core ? "" : "$".concat(_pre);
...
var updateProps = function updateProps(_ref2) {
  ...
  for (var pre in plugins) {
    ...
    props.push("mergeProps(\"".concat(pre, "\", ").concat(pre, ", ").concat(json.core ? "true" : "false", "), ").concat(json.namespace !== null ? "\"".concat(json.namespace, "\"") : "null"));
  }
...
var getJSON = function getJSON(_ref3) {
  ...
  var json_path = xNil(tar_path) ? resolve("".concat(tar_path, "/nextdapp.json")) : resolve("nd/".concat(pre, "/nextdapp.json"));
  var json = null;
